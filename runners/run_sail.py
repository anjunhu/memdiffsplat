"""SAIL unlearning runner for diffsplat-memeval."""
...
from unlearning import SAILOptimizer
...
def main(args):
    ...
    optimizer = SAILOptimizer(adapter, optim_steps=50, lr=0.08)
    ...
    for dataset in default_datasets():
        for idx, row in enumerate(tqdm(load_prompts(dataset), desc=dataset['name'])):
            ...
            for seed in range(4):
                out_dir = os.path.join("output/sail", dataset['name'],
            ...
            cross = os.path.join("output/sail", dataset['name'],
