"""AMG runner for diffsplat-memeval."""
...
from unlearning.amg_utils import NearestNeighborSearch
...
def main(args):
    ...
    nn_search = NearestNeighborSearch(device=device)
    dummy_imgs = [Image.new('RGB', (256, 256), 255) for _ in range(10)]
    nn_search.precompute_training_embeddings(dummy_imgs, [f"ref_{i}" for i in range(10)])
    ...
    for dataset in default_datasets():
        for idx, row in enumerate(tqdm(load_prompts(dataset), desc=dataset['name'])):
            ...
            all_images = []
            for seed in range(4):
                out_dir = os.path.join("output/amg", dataset['name'],
            ...
            cross = os.path.join("output/amg", dataset['name'],
